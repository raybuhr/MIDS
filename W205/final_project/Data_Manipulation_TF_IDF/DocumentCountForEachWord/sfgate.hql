...
--sfgate.hql
--Hive script for a mapreduce job to count how many document contains each word in Sf Gate article table.

drop table if exists texts;

--create a table from the source DynamoDb table. In this case, source is articles from SF_Gate table and "text" is the column which contains article body. 
CREATE EXTERNAL TABLE texts (text string)
STORED BY 'org.apache.hadoop.hive.dynamodb.DynamoDBStorageHandler' 
TBLPROPERTIES ("dynamodb.table.name" = "SF_Gate", 
...
STORED BY 'org.apache.hadoop.hive.dynamodb.DynamoDBStorageHandler' 
TBLPROPERTIES ("dynamodb.table.name" = "SF_GateWordCount", 
